...
# This is setting global variables that are used in the training process.
# reconizeWav file is where we store the audio data that we are training on - the additional data

#Used for inference
dataset=recognizeWav

# Sft thi sstsomewhere where you want to put your data, or where
...
# Creating a directory externally so that we can keep track of the work progress if the server UI closes
# We make subdirectories in the CheckPoints folder to mark how far along the program progressed.
rm -R CheckPoints
mkdir CheckPoints

<<"COMMENT"

# This code will prepare the Language Model. The LM is a set of vocabulary that the program can predict sounds to. Only words in the LM can be predicted.
# Check out the LM data in data/local/dict_nosp. New words must be added by hand.
...
# Here, we have the fix_data_dir command. This parses through our data in the train_10k directory.
# It will remove redundancies, as well as datapoints that aren't perfectly represented.
# In order for the datapoint to be registered, it needs to be represented in the utt2spk, spk2utt, utt2dur, text, spk2gender, wav.scp files.
# These fiels can be found in the data/train_10k folder.

# INSET dev_clean test_clean dev_other test_other train_clean_100
 utils/fix_data_dir.sh data/train_10k

# An MFCC stands for the Mel Frequency Cepestral Coefficient
# Each audio files is converted into an MFCC that is represented by n number of datapoints. This the data point the model is trained on.
# The MFCC file is stored in data/train_10k
for part in train_10k; do
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 40 data/$part exp/make_mfcc/$part $mfccdir
  steps/compute_cmvn_stats.sh data/$part exp/make_mfcc/$part $mfccdir
done

utils/validate_data_dir.sh data/train_10k

mkdir CheckPoints/MFCC1
<<"COMMENT"
...
# This is where data aligning happens
mkdir CheckPoints/Aligning
steps/align_si.sh --nj 10 --cmd "$train_cmd" \
 data/train_10k data/lang_nosp exp/tri1 exp/tri1_ali_10k


# This is where training happens. We are using an LDA+MLLT model.
# Trains using the train_10k dataset, the lang_nosp LM, and the tri2b neural net.
...
mkdir CheckPoints/DoneTraining
...
<<"COMMENT"
for test in $dataset; do
    steps/decode.sh --nj 1 --cmd "$decode_cmd" exp/tri2b/graph_nosp_tgsmall \
      data/$test exp/tri2b/decode_nosp_tgsmall_$test
    echo "hello world"
